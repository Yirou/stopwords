        ...
        getStopWords: function () {
            ...
            try {
                files.forEach(file => {
                    ...
                    result[filename] = fs.readFileSync(stopWordFilesDirectory + file, 'utf8').split('\n');
                });
            } catch (e) {
            }
            if (userWords.length)
        ...
        isStopWord: function (value) {
            try {
                ...
                for (let i = 0; i < files.length; i++) {
                    let stopWords = fs.readFileSync(stopWordFilesDirectory + files[i], 'utf8').split('\n');
        ...
        include: function (datas/*array of csv files or just data*/) {
            try {
                ...
                for (let entry of datas) {
                    ...
                    let encoding = typeof entry === 'object' && entry.encoding ? entry.encoding : 'utf8';
